...
class player():
    ...
    def move_up(self):
        if not self.moving:
            ...
            if (self.x, self.y - 1) not in globals.blocked_coordinates: 
                self.old_y, self.tote_y = self.y, int(self.y - 1)
                self.moving = True
    ...
    def move_right(self):
        if not self.moving:
            ...
            if (self.x + 1, self.y) not in globals.blocked_coordinates: 
                self.old_x, self.tote_x = self.x, int(self.x + 1)
                self.moving = True
    ...
    def pick_up_box(self):
        ...
        if not self.carrying_box:
            for box in globals.totes:
                if not box.shelved and ((box.x, box.y) == (self.x, self.y) or (box.x, box.y) == (self.nose_x, self.nose_y)):
    ...
    def pack_shipment(self):
        ...
        for dropoff_point in globals.dropoff_points:
            if ((dropoff_point.x, dropoff_point.y) == (int(self.nose_x), int(self.nose_y))):
                ...
                self.score += self.carrying_inventory
                self.carrying_inventory = 0
    ...
    def update_position(self):
        if self.x < self.tote_x-.1: self.x += self.speed
        elif self.x > self.tote_x+.1: self.x -= self.speed
        elif self.y < self.tote_y-.1: self.y += self.speed
        elif self.y > self.tote_y+.1: self.y -= self.speed
        else:
            self.x, self.y = self.tote_x, self.tote_y
            self.set_nose()
            self.moving = False
            if self.role == 'packer': 
                self.pick_up_box()
...
class tote():
    ...
    def draw(self, canvas):
        if not self.collected:
            ...
            canvas.draw_text(str(self.inventory), [g*self.x - 5, g*self.y + 5], 8, 'blue')
